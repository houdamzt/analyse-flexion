...
labels = {
    "Français": {
        ...
        "without_gamma": "Sans gamma (γ = 0°)",
        "with_gamma": "Avec gamma (γ = {gamma:.1f}°)",
        "alpha_measured": "Alpha mesuré",
        "alpha_crit": "Alpha critique",
    ...
    "English": {
        ...
        "without_gamma": "Without gamma (γ = 0°)",
        "with_gamma": "With gamma (γ = {gamma:.1f}°)",
        "alpha_measured": "Alpha measured",
        "alpha_crit": "Critical alpha",
...
if alpha_critique_avec_gamma == 129.9:
    interpretation_alpha_avec = "Pas de risque de luxation : Réserve infinie"
else:
    interpretation_alpha_avec = "Risque de luxation : Réserve limitée"
...
if alpha_critique_sans_gamma == 129.9:
    interpretation_alpha_sans = "Pas de risque de luxation : Réserve infinie"
else:
    interpretation_alpha_sans = "Risque de luxation : Réserve limitée"
...
with col3:
    st.markdown("**" + ("Sans gamma (γ = 0°)" if langue == "Français" else "Without gamma (γ = 0°)") + "**")
    st.write(f"• {'Alpha mesuré' if langue == 'Français' else 'Measured alpha'} : :green[{alpha_mesure:.1f}°]")
    st.write(f"• {'Alpha critique' if langue == 'Français' else 'Critical alpha'} : :green[{alpha_critique_sans_gamma:.1f}°]")
    st.write(f"• {'Réserve de flexion' if langue == 'Français' else 'Flexion reserve'} : :green[{reserve_alpha_sans_gamma:.1f}°]")
    st.markdown(f"*{interpretation_alpha_sans}*")

with col4:
    st.markdown("**" + (f"Avec gamma (γ = {gamma_final:.1f}°)" if langue == "Français" else f"With gamma (γ = {gamma_final:.1f}°)") + "**")
    st.write(f"• {'Alpha mesuré' if langue == 'Français' else 'Measured alpha'} : :green[{alpha_mesure:.1f}°]")
    st.write(f"• {'Alpha critique' if langue == 'Français' else 'Critical alpha'} : :green[{alpha_critique:.1f}°]")
    st.write(f"• {'Réserve de flexion' if langue == 'Français' else 'Flexion reserve'} : :green[{reserve_alpha:.1f}°]")
    st.markdown(f"*{interpretation_alpha}*")
...
if st.button(labels["export_pdf"]):
    ...
    # --- Résultats flexion jambe ---
    pdf.multi_cell(0, 10, "Résultats flexion jambe")
    pdf.multi_cell(0, 10, f"Sans gamma (γ = 0°):")
    pdf.multi_cell(0, 10, f"Alpha mesuré : {alpha_mesure:.1f}°")
    pdf.multi_cell(0, 10, f"Alpha critique : {alpha_critique_sans_gamma:.1f}°")
    pdf.multi_cell(0, 10, f"Réserve de flexion : {reserve_alpha_sans_gamma:.1f}°")
    pdf.multi_cell(0, 10, interpretation_alpha_sans)

    pdf.multi_cell(0, 10, f"Avec gamma (γ = {gamma_final:.1f}°):")
    pdf.multi_cell(0, 10, f"Alpha mesuré : {alpha_mesure:.1f}°")
    pdf.multi_cell(0, 10, f"Alpha critique : {alpha_critique_avec_gamma:.1f}°")
    pdf.multi_cell(0, 10, f"Réserve de flexion : {reserve_alpha_avec_gamma:.1f}°")
    pdf.multi_cell(0, 10, interpretation_alpha_avec)
