...
alpha_critique = calcul_alpha_critique(tf, ccd, offset, anteversion, gamma_final)
reserve_alpha = alpha_critique - alpha_mesure

if alpha_critique == 129.9:
    interpretation_alpha = "Pas de risque de luxation : Réserve infinie"
else:
    interpretation_alpha = "Risque de luxation : Réserve limitée"
...
st.subheader("Résultats flexion jambe")

col3, col4 = st.columns(2)

with col3:
    st.markdown(" **Sans gamma (γ = 0°)**")
    st.write(f"• Alpha mesuré : :green[{alpha_mesure:.1f}°]")
    st.write(f"• Alpha critique : :green[{alpha_critique_sans_gamma:.1f}°]")
    st.write(f"• Réserve de flexion : :green[{reserve_alpha_sans_gamma:.1f}°]")
    st.markdown(f"*{interpretation_alpha_sans}*")

with col4:
    st.markdown(f" **Avec gamma (γ = {gamma_final:.1f}°)**")
    st.write(f"• Alpha mesuré : :green[{alpha_mesure:.1f}°]")
    st.write(f"• Alpha critique : :green[{alpha_critique_avec_gamma:.1f}°]")
    st.write(f"• Réserve de flexion : :green[{reserve_alpha_avec_gamma:.1f}°]")
