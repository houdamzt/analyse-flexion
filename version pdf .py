...
if st.button(labels["export_pdf"]):
    ...
    pdf.ln(5)
    pdf.set_font("Arial", "B", 14)
    pdf.cell(0, 10, "🦵 " + ("Flexion de la jambe" if langue == "Français" else "Leg flexion"), ln=1)

    pdf.ln(3)
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, f"Sans gamma (γ = 0°):", ln=1)
    pdf.set_font("Arial", "", 12)
    ...
    pdf.multi_cell(0, 8, f"{'Réserve de flexion' if langue == 'Français' else 'Flexion reserve'}: {reserve_alpha_sans_gamma:.1f}°")
    pdf.multi_cell(0, 8, interpretation_alpha_sans)

    pdf.ln(3)
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, f"Avec gamma (γ = {gamma_final:.1f}°):", ln=1)
    pdf.set_font("Arial", "", 12)
    pdf.multi_cell(0, 8, f"{labels['alpha_measured']}: {alpha_mesure:.1f}°")
    pdf.multi_cell(0, 8, f"{labels['alpha_crit']}: {alpha_critique_avec_gamma:.1f}°")
    pdf.multi_cell(0, 8, f"{'Réserve de flexion' if langue == 'Français' else 'Flexion reserve'}: {reserve_alpha_avec_gamma:.1f}°")
